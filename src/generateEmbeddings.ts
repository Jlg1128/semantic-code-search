...
import { DEFAULT_BATCH_COUNT, dataFilePathGetter } from './const';
  ...
  async function getCodeEmbeddings() {
    ...
    for (const data of datas as CSVData) {
      if (i % batchCount === 0) {
        ...
        start += batchCount;
        const embeddingResponseData = await getEmbedding(batchCodes);
        if (embeddingResponseData) {
          ...
          for (const count of [first, sencond]) {
            if (count > 0) {
              const embeddingResponseData = await getEmbedding(batchCodes.slice(retryBegin, retryBegin + count));
              retryBegin += count;
              if (embeddingResponseData) {
                batchGenerate(batchDatas, embeddingResponseData);
              }
